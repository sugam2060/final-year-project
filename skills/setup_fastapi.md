...
1. **Environment Variables (`src/.env`):**
   - Populate the `.env` file with the following tokens:
     ...
     GITHUB_TOKEN
     OPENAI_API_KEY
     LLM_PROVIDER=NVIDIA # Options: NVIDIA, OPENAI
...
2.  **Configuration (`src/config.py`):**
    ...
    - Code requirement:
      ...
      NVIDIA_API_KEY = config("NVIDIA_API_KEY", cast=Secret)
...
4.  **Webhook Router (`src/router/webhook.py`):**
    ...
    - Implement two handlers:
      - `_handle_pull_request_event`: For initial reviews on `opened`, `reopened`, and `synchronize` PR events.
      - `_handle_issue_comment_event`: For conversational reviews triggered by `@swarm`.
      - `_handle_review_comment_event`: Optimized inline replies. Extract `path` and `diff_hunk` from the payload to provide localized code context to the agent.
...
6.  **Requirements (`src/requirements.txt`):**
    - Include `fastapi`, `uvicorn`, `starlette`, `pydantic`, `pydantic-settings`, `langgraph`, `langchain-core`, `langchain-openai`, `langchain-nvidia-ai-endpoints`, `PyGithub`, `langchain-mcp-adapters`, and `mcp`.
